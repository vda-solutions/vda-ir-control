...
class VDAIRDeviceChannelsView(HomeAssistantView):
    """View for controlled device channel presets."""

    url = "/api/vda_ir_control/devices/{device_id}/channels"
    name = "api:vda_ir_control:device_channels"
    requires_auth = True

    async def get(self, request, device_id):
        """Get channel presets for a controlled device."""
        hass = request.app["hass"]
        storage = get_storage(hass)
        ...
        return self.json({
            "device_id": device_id,
            "channels": [ch.to_dict() for ch in device.channels],
        })

    async def put(self, request, device_id):
        """Update channel presets for a controlled device."""
        hass = request.app["hass"]
        storage = get_storage(hass)
        device = await storage.async_get_device(device_id)
        ...
        device.channels = [ChannelPreset.from_dict(ch) for ch in channels_data]

        await storage.async_save_device(device)
        _LOGGER.info("Updated channels for device: %s (%d channels)", device_id, len(device.channels))
    ...
    async def get(self, request):
        ...
        for group in groups:
            result.append({
                ...
                "power_on_repeat": group.power_on_repeat,
                "power_off_repeat": group.power_off_repeat,
            })
...
class VDAIRDeviceGroupView(HomeAssistantView):
    ...
    async def put(self, request, group_id):
        ...
        if "power_on_repeat" in data:
            group.power_on_repeat = data["power_on_repeat"]
        if "power_off_repeat" in data:
            group.power_off_repeat = data["power_off_repeat"]
        if "members" in data:
            group.members = [
                DeviceGroupMember(
                    device_id=m["device_id"],
...
class VDAIRGroupPowerView(HomeAssistantView):
    """API endpoint for group power control (discrete on/off)."""

    url = "/api/vda_ir_control/device_groups/{group_id}/power"
    name = "api:vda_ir_control:device_group_power"
    requires_auth = True

    async def post(self, request, group_id):
        """Send power on/off command to all devices in group."""
        hass = request.app["hass"]
        storage = get_storage(hass)
        ...
        if action not in ("on", "off"):
            return self.json({"error": "action must be 'on' or 'off'"}, status_code=400)

        # Determine command based on action
        primary_command = f"power_{action}"  # power_on or power_off
        fallback_command = "power_toggle"

        # Get repeat count for discrete commands
        repeat_count = group.power_on_repeat if action == "on" else group.power_off_repeat
        repeat_delay_ms = 100  # Delay between repeated commands to same device

        results = []
        delay_ms = group.sequence_delay_ms or 20

        for i, member in enumerate(group.members):
            # Add delay between commands (except first)
            if i > 0:
                await asyncio.sleep(delay_ms / 1000.0)

            result = {"device_id": member.device_id, "device_type": member.device_type}
            ...
            try:
                if member.device_type == "controlled":
                    ...
                    # Get profile to check available commands
                    command_to_send = await self._get_best_command(
                        hass, storage, device.device_profile_id, primary_command, fallback_command
                    )

                    if command_to_send:
                        # Determine how many times to send
                        # Only repeat discrete commands (power_on, power_off), not toggle
                        is_discrete = command_to_send in ("power_on", "power_off")
                        times_to_send = repeat_count if is_discrete else 1

                        for send_idx in range(times_to_send):
                            if send_idx > 0:
                                await asyncio.sleep(repeat_delay_ms / 1000.0)
                            await hass.services.async_call(
                                DOMAIN,
                                "send_command",
                                {"device_id": member.device_id, "command": command_to_send},
                                blocking=True,
                            )
                        result["success"] = True
                        result["command"] = command_to_send
                        result["repeat_count"] = times_to_send
                    else:
                        result["success"] = False
                        result["error"] = "No power command available"

                elif member.device_type == "serial":
                    # Serial device - check for power_on/power_off/power_toggle commands
                    serial_device = await storage.async_get_serial_device(member.device_id)
                    ...
                    commands = serial_device.commands or {}
                    command_id = None
                    is_discrete_serial = False

                    # Find the command to send
                    for cmd_id, cmd in commands.items():
                        cmd_name = cmd.get("name", "").lower()
                        if cmd_name == primary_command.replace("_", " "):
                            command_id = cmd_id
                            is_discrete_serial = True
                            break
                        elif cmd_name == fallback_command.replace("_", " "):
                            command_id = cmd_id
                            is_discrete_serial = False
                            # Keep looking for primary

                    if command_id:
                        times_to_send = repeat_count if is_discrete_serial else 1
                        for send_idx in range(times_to_send):
                            if send_idx > 0:
                                await asyncio.sleep(repeat_delay_ms / 1000.0)
                            await hass.services.async_call(
                                DOMAIN,
                                "send_serial_command",
                                {"device_id": member.device_id, "command_id": command_id},
                                blocking=True,
                            )
                        result["success"] = True
                ...
                elif member.device_type == "ha":
                    # Home Assistant remote entity
                    ha_device = await storage.async_get_ha_device(member.device_id)
                    if not ha_device:
                        result["success"] = False
                        result["error"] = "Device not found"
                        results.append(result)
                    ...
                    result["success"] = False
                    result["error"] = f"Unknown device type: {member.device_type}"

            except Exception as err:
                _LOGGER.error("Failed to send power command to %s: %s", member.device_id, err)
                result["success"] = False
    ...
    async def _get_best_command(self, hass, storage, profile_id, primary, fallback):
        """Get the best available command from profile (primary or fallback)."""
        ...
        if profile_id.startswith("builtin:"):
            ...
            if profile:
                ...
                raw_codes = profile.get("raw_codes", {})
                all_commands = set(codes.keys()) | set(raw_codes.keys())

                if primary in all_commands:
                    return primary
                if fallback in all_commands:
                    return fallback
                # Also try just "power" as final fallback
                if "power" in all_commands:
                    return "power"
            return None
            ...
            profile_manager = get_profile_manager(hass)
            await profile_manager.async_load()
            profile = profile_manager.get_community_profile(community_id)
            ...
            profile = await storage.async_get_profile(profile_id)
            if profile:
                commands = profile.get_learned_commands()
...
class VDAIRGroupChannelsView(HomeAssistantView):
    """API endpoint for group channel presets."""

    url = "/api/vda_ir_control/device_groups/{group_id}/channels"
    name = "api:vda_ir_control:device_group_channels"
    requires_auth = True

    async def get(self, request, group_id):
        """Get channel presets for a device group."""
        hass = request.app["hass"]
        storage = get_storage(hass)
        group = await storage.async_get_device_group(group_id)
    ...
    async def put(self, request, group_id):
        """Update channel presets for a device group."""
        hass = request.app["hass"]
        storage = get_storage(hass)
        group = await storage.async_get_device_group(group_id)
        ...
        from .models import ChannelPreset
        channels_data = data.get("channels", [])
        group.channels = [ChannelPreset.from_dict(ch) for ch in channels_data]

        await storage.async_save_device_group(group)
        _LOGGER.info("Updated channels for device group: %s (%d channels)", group_id, len(group.channels))
...
class VDAIRHADeviceChannelsView(HomeAssistantView):
    """View for HA device channel presets."""

    url = "/api/vda_ir_control/ha_devices/{device_id}/channels"
    name = "api:vda_ir_control:ha_device_channels"
    requires_auth = True

    async def get(self, request, device_id):
        """Get channel presets for an HA device."""
        hass = request.app["hass"]
        storage = get_storage(hass)
        device = await storage.async_get_ha_device(device_id)
    ...
    async def put(self, request, device_id):
        """Update channel presets for an HA device."""
        hass = request.app["hass"]
        storage = get_storage(hass)
        device = await storage.async_get_ha_device(device_id)
        ...
        await storage.async_save_ha_device(device)
        _LOGGER.info("Updated channels for HA device: %s (%d channels)", device_id, len(device.channels))
...
class VDAIRChannelLogoView(HomeAssistantView):
    """API endpoint for channel logos."""

    url = "/api/vda_ir_control/logos/{logo_name}"
    name = "api:vda_ir_control:channel_logo"
    requires_auth = False  # Logos can be public

    async def get(self, request, logo_name):
        """Serve a channel logo image."""
        import os
        from aiohttp import web

        # Security: only allow alphanumeric, dash, underscore
        if not all(c.isalnum() or c in '-_' for c in logo_name.replace('.png', '')):
            return web.Response(status=400, text="Invalid logo name")

        # Ensure .png extension
        if not logo_name.endswith('.png'):
            logo_name = f"{logo_name}-us.png"
...
class VDAIRChannelLogosListView(HomeAssistantView):
    """API endpoint to list available channel logos."""

    url = "/api/vda_ir_control/logos"
    name = "api:vda_ir_control:channel_logos"
    requires_auth = True

    async def get(self, request):
        """List all available channel logos."""
        import os
        ...
        if os.path.isdir(logo_dir):
            for f in os.listdir(logo_dir):
                if f.endswith('.png') and f not in ('icon.png', 'icon_dark.png'):
                    # Extract channel name from filename (remove -us.png suffix)
                    name = f.replace('-us.png', '').replace('.png', '')
                    logos.append({
                        "name": name,
                        "filename": f,
                        "url": f"/api/vda_ir_control/logos/{f}",
                    })

        return self.json({"logos": sorted(logos, key=lambda x: x['name'])})


class VDAIRGlobalChannelPresetsView(HomeAssistantView):
    """API endpoint for global channel presets (shared across all DTV sources)."""

    url = "/api/vda_ir_control/channel_presets"
    name = "api:vda_ir_control:channel_presets"
    requires_auth = True

    async def get(self, request):
        """Get global channel presets."""
        hass = request.app["hass"]
        storage = get_storage(hass)
        channels = await storage.async_get_channel_presets()
    ...
    async def put(self, request):
        ...
        from .models import ChannelPreset
        channels_data = data.get("channels", [])
        channels = [ChannelPreset.from_dict(ch) for ch in channels_data]

        await storage.async_save_channel_presets(channels)
        _LOGGER.info("Updated global channel presets (%d channels)", len(channels))
...
async def async_setup_api(hass: HomeAssistant) -> None:
    ...
    hass.http.register_view(VDAIRDeviceChannelsView())
    ...
    hass.http.register_view(VDAIRGroupPowerView())
    hass.http.register_view(VDAIRGroupChannelsView())
    ...
    hass.http.register_view(VDAIRHADeviceChannelsView())
    ...
    # Channel logo endpoints
    hass.http.register_view(VDAIRChannelLogosListView())
    hass.http.register_view(VDAIRChannelLogoView())

    # Global channel presets
    hass.http.register_view(VDAIRGlobalChannelPresetsView())
