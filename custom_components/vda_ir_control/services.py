...
CREATE_DEVICE_SCHEMA = vol.Schema({
    ...
    vol.Optional("linked_screen_id"): vol.Any(str, None),
    vol.Optional("screen_down_delay"): vol.Any(float, None),
})
...
async def async_setup_services(hass: HomeAssistant) -> None:
    ...
    async def handle_create_device(call: ServiceCall) -> Dict[str, Any]:
        ...
        device = ControlledDevice(
            ...
            linked_screen_id=call.data.get("linked_screen_id"),
            screen_down_delay=call.data.get("screen_down_delay"),
        )
    ...
    async def handle_update_device(call: ServiceCall) -> Dict[str, Any]:
        ...
        # Update screen link fields
        if "linked_screen_id" in call.data:
            device.linked_screen_id = call.data["linked_screen_id"]
        if "screen_down_delay" in call.data:
            device.screen_down_delay = call.data["screen_down_delay"]
    ...
    async def handle_send_command(call: ServiceCall) -> Dict[str, Any]:
        ...
        # Check if any screens are linked to this device (screens store the link to their trigger device)
        if command in ["power_on", "power_off"]:
            all_devices = await storage.async_get_all_devices()
            linked_screens = [d for d in all_devices if d.linked_screen_id == device_id]

            for screen in linked_screens:
                screen_coordinator = _get_board_coordinator(screen.board_id)
                if not screen_coordinator:
                    continue

                # Get screen IR codes
                screen_profile_id = screen.device_profile_id
                screen_ir_code = None
                screen_protocol = None
                screen_raw_data = None
                screen_frequency = 38000

                screen_command = "down" if command == "power_on" else "up"

                if screen_profile_id.startswith("builtin:"):
                    builtin_id = screen_profile_id[8:]
                    screen_profile = get_builtin_profile(builtin_id)
                    if screen_profile is None:
                        profile_manager = get_profile_manager(hass)
                        await profile_manager.async_load()
                        screen_profile = profile_manager.get_community_profile(builtin_id)

                    if screen_profile:
                        raw_codes = screen_profile.get("raw_codes", {})
                        codes = screen_profile.get("codes", {})
                        screen_frequency = screen_profile.get("frequency", 38000)

                        if screen_command in raw_codes:
                            screen_ir_code = codes.get(screen_command, "0")
                            screen_protocol = "raw"
                            screen_raw_data = raw_codes[screen_command]
                        elif screen_command in codes:
                            screen_ir_code = codes[screen_command]
                            screen_protocol = screen_profile.get("protocol", "NEC")

                elif screen_profile_id.startswith("community:"):
                    community_id = screen_profile_id[10:]
                    profile_manager = get_profile_manager(hass)
                    await profile_manager.async_load()
                    screen_profile = profile_manager.get_community_profile(community_id)
                    ...
                    # User profile
                    screen_profile = await storage.async_get_profile(screen_profile_id)
                    if screen_profile:
                        ir_code_obj = screen_profile.get_code(screen_command)
                        if ir_code_obj:
                            screen_ir_code = ir_code_obj.raw_code
                            screen_protocol = ir_code_obj.protocol
                            screen_frequency = ir_code_obj.frequency
                            if screen_protocol == "raw" and ir_code_obj.raw_code:
                                try:
                                    screen_raw_data = [int(x) for x in ir_code_obj.raw_code.split()]
                                except ValueError:
                                    pass

                # Send screen command
                if screen_ir_code:
                    _LOGGER.info("Triggering linked screen %s: %s (triggered by %s)", screen.device_id, screen_command, device_id)
                    await screen_coordinator.send_ir_code(
                        screen.output_port, screen_ir_code, screen_protocol,
                        screen_raw_data, screen_frequency
                    )

                    # For power_on (screen down), wait and then send stop
                    if command == "power_on" and screen.screen_down_delay and screen.screen_down_delay > 0:
                        _LOGGER.info("Waiting %.2f seconds before stopping screen", screen.screen_down_delay)
                        await asyncio.sleep(screen.screen_down_delay)
                        ...
                        if screen_profile_id.startswith("builtin:") or screen_profile_id.startswith("community:"):
                            if screen_profile:
                                raw_codes = screen_profile.get("raw_codes", {})
                                codes = screen_profile.get("codes", {})

                                if "stop" in raw_codes:
                                    stop_ir_code = codes.get("stop", "0")
                                    stop_protocol = "raw"
                                    stop_raw_data = raw_codes["stop"]
                                elif "stop" in codes:
                                    stop_ir_code = codes["stop"]
                                    stop_protocol = screen_profile.get("protocol", "NEC")
                        else:
                            if screen_profile:
                                stop_code_obj = screen_profile.get_code("stop")
                                if stop_code_obj:
                                    stop_ir_code = stop_code_obj.raw_code
                                    stop_protocol = stop_code_obj.protocol
                                    if stop_protocol == "raw" and stop_code_obj.raw_code:
                                        try:
                                            stop_raw_data = [int(x) for x in stop_code_obj.raw_code.split()]
                                        except ValueError:
                                            pass

                        if stop_ir_code:
                            _LOGGER.info("Sending stop command to screen %s", screen.device_id)
                            await screen_coordinator.send_ir_code(
                                screen.output_port, stop_ir_code, stop_protocol,
                                stop_raw_data, screen_frequency
                            )
    ...
    async def handle_send_serial_command(call: ServiceCall) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check if any screens are linked to this serial device
            cmd_lower = command_id.lower()
            is_power_on = "power_on" in cmd_lower or (cmd_lower == "on")
            is_power_off = "power_off" in cmd_lower or (cmd_lower == "off")

            if is_power_on or is_power_off:
                storage = get_storage(hass)
                all_devices = await storage.async_get_all_devices()
                linked_screens = [d for d in all_devices if d.linked_screen_id == device_id]
                ...
                for screen in linked_screens:
                    ...
                    screen_raw_data = None
                    screen_frequency = 38000
                    screen_command = "down" if is_power_on else "up"
                    ...
                    if screen_ir_code:
                        _LOGGER.info("Triggering linked screen %s: %s (triggered by serial device %s)", screen.device_id, screen_command, device_id)
                        await screen_coordinator.send_ir_code(
                            screen.output_port, screen_ir_code, screen_protocol,
                            screen_raw_data, screen_frequency
                        )

                        if is_power_on and screen.screen_down_delay and screen.screen_down_delay > 0:
                            _LOGGER.info("Waiting %.2f seconds before stopping screen", screen.screen_down_delay)
                            await asyncio.sleep(screen.screen_down_delay)
