...
STORAGE_KEY_CHANNEL_PRESETS = f"{DOMAIN}_channel_presets"
...
class VDAIRStorage:
    ...
    def __init__(self, hass: HomeAssistant):
        ...
        self._channel_presets_store = Store(hass, STORAGE_VERSION, STORAGE_KEY_CHANNEL_PRESETS)
        ...
        self._channel_presets: List[ChannelPreset] = []
        self._loaded = False
    ...
    async def async_load(self) -> None:
        ...
        # Load global channel presets
        channel_presets_data = await self._channel_presets_store.async_load()
        if channel_presets_data and isinstance(channel_presets_data, list):
            for preset_dict in channel_presets_data:
                try:
                    self._channel_presets.append(ChannelPreset.from_dict(preset_dict))
                except Exception as err:
                    _LOGGER.error("Failed to load channel preset: %s", err)

        self._loaded = True
        _LOGGER.info(
            "Loaded %d boards, %d profiles, %d IR devices, %d serial devices, %d device groups, %d HA devices, %d channel presets",
            len(self._boards),
            len(self._profiles),
            len(self._devices),
    ...
    async def _async_save_channel_presets(self) -> None:
        """Save channel presets to storage."""
        data = [ch.to_dict() for ch in self._channel_presets]
        await self._channel_presets_store.async_save(data)

    async def async_get_channel_presets(self) -> List[ChannelPreset]:
        """Get all global channel presets."""
        await self.async_load()
        return list(self._channel_presets)

    async def async_save_channel_presets(self, channels: List[ChannelPreset]) -> None:
        """Save global channel presets (replaces all)."""
        await self.async_load()
        self._channel_presets = list(channels)
        await self._async_save_channel_presets()
